...
namespace Expresspay\Epos\Controller\Url;
...
class Submit extends Action implements CsrfAwareActionInterface
{
    ...
    /**
     * @var \Expresspay\Epos\Block\Widget\Redirect
     */
    public $epos;
    ...
    public function __construct(
        ...
        \Expresspay\Epos\Block\Widget\Redirect $epos_form,
    ...
    public function execute()
    {
        ...
        $post_data = $this->epos->getPostData($data);

        $request = $this->doRequest($post_data, $this->epos->getGateUrl());
        ...
        if (isset($response['Errors'])) {
            ...
            $this->_coreRegistry->register('path', $this->epos->getPath());
            $this->_coreRegistry->register('ExpressPayInvoiceNo', $response['ExpressPayInvoiceNo']);
            $this->_coreRegistry->register('Amount', $this->epos->getAmount());

            $resultPage = $this->resultPageFactory->create();
            $resultPage->getConfig()->getTitle()->set(__('Счет выставлен в системе E-POS'));
            return $resultPage;
        }
    }
